...
void inputBufferHandler(void *inUserData, AudioQueueRef inAQ, AudioQueueBufferRef inBuffer, const AudioTimeStamp *inStartTime,UInt32 inNumPackets, const AudioStreamPacketDescription *inPacketDesc)
{
    ...
    if([recorder.delegate respondsToSelector:@selector(audioRecorder:recordTime:)])
    {
        NSDate *curDate = [NSDate date];
        NSInteger duration = [curDate timeIntervalSinceDate:recorder.startDate];;
        [recorder.delegate audioRecorder:RecordTypePrepareToRecord recordTime:duration];
    }
    if (inNumPackets > 0) {
